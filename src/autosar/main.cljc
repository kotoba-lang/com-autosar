(ns autosar.main
  "Kotodama WASM entrypoint for the Autosar clean-room actor (L4) — Clojure port.
  ...
  facts (`autosar.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "autosar")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"autosar.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "autosar-compat" :tier tier :entities entities} 200])
...
(defn start! [] :autosar-compat/ready)
